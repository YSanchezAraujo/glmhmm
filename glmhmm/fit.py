...
def fit_glmhmm(
    ...
    This function can fit:
    - A Bernoulli GLM-HMM.
    - A Gaussian GLM-HMM.
    - A joint Bernoulli-Gaussian GLM-HMM.
    ...
    Returns:
        A NamedTuple containing the fitted parameters (A, pi0, W_bern, W_gauss,
        Sigma), posterior probabilities (gamma), log-marginal likelihood (lml),
        and trial/sequence information.
    """
    ...
    if has_bern and has_gauss:
        ...
        for i in range(batch_size):
            gauss_trials = len(x_nested_set[i])
            if x_set_bern[i].shape[0] != gauss_trials:
                 raise ValueError(f"Sequence {i}: Bernoulli length ({x_set_bern[i].shape[0]}) "
                                  f"must match Gaussian trial count ({gauss_trials}).")
    elif has_bern:
    ...
    A = jnp.ones((n_states, n_states)) + dirichlet_prior
    ...
    if has_bern:
        ...
        W_bern_init, _ = bern_init_opt(x_concat_bern, y_concat_bern)
    ...
    neural_trial_info = None
    first_idx_set, last_idx_set = None, None
    n_feat_gauss, n_reg_gauss = 0, 0
    if has_gauss:
    ...
    for k in tqdm(range(max_iter), desc="EM Iteration"):
        ...
        lml_total = 0.0
        gamma_set_gauss_expanded = []

        # E-step
        for i in range(batch_size):
            seq_len = 0
            if has_bern:
                seq_len = x_set_bern[i].shape[0]
            elif has_gauss:
                seq_len = len(first_idx_set[i])

            ll = jnp.zeros((seq_len, n_states))

            if has_bern:
                ll += bernoulli_glmhmm_loglikelihood(x_set_bern[i], y_set_bern[i], W_bern)
            ...
            if has_gauss:
                ...
                gamma_set_gauss_expanded.append(expanded_i)
        ...
        if has_bern:
            W_bern, _ = bern_m_step_optax(
                ...
                num_opt_steps=bern_m_step_maxiter, learning_rate=bern_lr_optax
            )

        if has_gauss:
            W_gauss, Sigma = batch_m_step_updates(
                x_set_gauss, y_set_gauss, gamma_set_gauss_expanded,
    ...
    if has_gauss:
        output_fields.extend(["W_gauss", "Sigma", "trial_info"])
        output_values.extend([W_gauss, Sigma, neural_trial_info])
